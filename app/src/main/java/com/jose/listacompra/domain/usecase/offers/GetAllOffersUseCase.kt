...
class GetAllOffersUseCase @Inject constructor(
    ...
    suspend operator fun invoke(): List<Offer> {
        android.util.Log.d("GetAllOffersUseCase", "🔍 Ejecutando getAllOffers...")
        var offers = offerRepository.getAllOffers()
        android.util.Log.d("GetAllOffersUseCase", "📊 Repository devolvió ${offers.size} offers")
        
        // Si no hay ofertas, crear las predefinidas
        if (offers.isEmpty()) {
            android.util.Log.d("GetAllOffersUseCase", "⚡ No hay ofertas, insertando defaults...")
            ...
            android.util.Log.d("GetAllOffersUseCase", "✅ Insertadas ${defaultOffers.size} offers")
        } else {
            android.util.Log.d("GetAllOffersUseCase", "✅ Ya existían ${offers.size} offers: ${offers.map { it.name }}")
        }
